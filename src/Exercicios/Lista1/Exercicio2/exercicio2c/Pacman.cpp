...
const GLuint NUM_SEGMENTOS = 100;
const GLuint NUM_VERTICES = NUM_SEGMENTOS + 2;
...
int main()
{
	...
	GLFWwindow *window = glfwCreateWindow(WIDTH, HEIGHT, "Exercicio 2c - Pac-man", nullptr, nullptr);
	if (!window)
	{
	...
	while (!glfwWindowShouldClose(window))
	{
		// Este trecho de código é totalmente opcional: calcula e mostra a contagem do FPS na barra de título
		{
			...
			if (title_countdown_s <= 0.0 && elapsed_s > 0.0)
			{
				...
				char tmp[256];
				sprintf(tmp, "Exercicio 2c - Pac-man\tFPS %.2lf", fps);
		...
		glUniform4f(colorLoc, 1.0f, 0.85f, 0.0f, 1.0f); // enviando cor para variável uniform inputColor

		// Chamada de desenho - drawcall
		// Pac-man preenchido, deixando uma abertura para a boca
...
int setupGeometry()
{
	...
	// Centro do Pac-man para o GL_TRIANGLE_FAN
	vertices.push_back(0.0f);
	vertices.push_back(0.0f);
	vertices.push_back(0.0f);

	const GLfloat raio = 0.65f;
	const GLfloat correcaoAspecto = static_cast<GLfloat>(HEIGHT) / WIDTH;
	const GLfloat anguloInicial = 35.0f * PI / 180.0f;
	const GLfloat anguloFinal = 325.0f * PI / 180.0f;

	// Gera o arco pela equacao parametrica do circulo.
	// O intervalo que nao e gerado forma a boca do Pac-man.
	for (GLuint i = 0; i <= NUM_SEGMENTOS; i++)
	{
		GLfloat angulo = anguloInicial +
			(anguloFinal - anguloInicial) * i / NUM_SEGMENTOS;
		vertices.push_back(raio * correcaoAspecto * std::cos(angulo));
		vertices.push_back(raio * std::sin(angulo));
		vertices.push_back(0.0f);
	}
