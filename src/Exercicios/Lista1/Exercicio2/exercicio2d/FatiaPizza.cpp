...
int main()
{
	...
	GLFWwindow *window = glfwCreateWindow(WIDTH, HEIGHT, "Exercicio 2d - Fatia de pizza", nullptr, nullptr);
	if (!window)
	{
	...
	while (!glfwWindowShouldClose(window))
	{
		// Este trecho de código é totalmente opcional: calcula e mostra a contagem do FPS na barra de título
		{
			...
			if (title_countdown_s <= 0.0 && elapsed_s > 0.0)
			{
				...
				char tmp[256];
				sprintf(tmp, "Exercicio 2d - Fatia de pizza\tFPS %.2lf", fps);
		...
		glUniform4f(colorLoc, 1.0f, 0.45f, 0.05f, 1.0f); // enviando cor para variável uniform inputColor

		// Chamada de desenho - drawcall
		// Fatia de pizza preenchida
...
int setupGeometry()
{
	...
	// Centro da fatia para o GL_TRIANGLE_FAN
	vertices.push_back(0.0f);
	vertices.push_back(0.0f);
	vertices.push_back(0.0f);

	const GLfloat raio = 0.70f;
	const GLfloat correcaoAspecto = static_cast<GLfloat>(HEIGHT) / WIDTH;
	const GLfloat anguloInicial = 25.0f * PI / 180.0f;
	const GLfloat anguloFinal = 145.0f * PI / 180.0f;

	// Gera o arco da fatia pela equacao parametrica do circulo
