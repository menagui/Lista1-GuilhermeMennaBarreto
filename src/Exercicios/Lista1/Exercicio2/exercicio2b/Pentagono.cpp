...
const GLuint NUM_LADOS = 5;
const GLuint NUM_VERTICES = NUM_LADOS + 2;
...
int main()
{
	...
	GLFWwindow *window = glfwCreateWindow(WIDTH, HEIGHT, "Exercicio 2b - Pentagono", nullptr, nullptr);
	if (!window)
	{
	...
	while (!glfwWindowShouldClose(window))
	{
		// Este trecho de código é totalmente opcional: calcula e mostra a contagem do FPS na barra de título
		{
			...
			if (title_countdown_s <= 0.0 && elapsed_s > 0.0)
			{
				...
				char tmp[256];
				sprintf(tmp, "Exercicio 2b - Pentagono\tFPS %.2lf", fps);
		...
		glUniform4f(colorLoc, 0.0f, 0.8f, 0.3f, 1.0f); // enviando cor para variável uniform inputColor

		// Chamada de desenho - drawcall
		// Pentagono preenchido
...
int setupGeometry()
{
	...
	// Centro do pentagono para o GL_TRIANGLE_FAN
	...
	// Gera os vertices pela equacao parametrica do circulo:
	// x = raio * cos(angulo) e y = raio * sin(angulo)
	for (GLuint i = 0; i <= NUM_LADOS; i++)
	{
		GLfloat angulo = PI / 2.0f + 2.0f * PI * i / NUM_LADOS;
